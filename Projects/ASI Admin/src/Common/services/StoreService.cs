        ...
        public void UpdateTaxAndShipping(StoreOrder order)
        {
            ...
            if (address != null && order != null && order.OrderDetails != null && order.OrderDetails.Count > 0)
            {
                ...
                foreach (StoreOrderDetail orderDetail in order.OrderDetails)
                {
                    ...
                    if (orderDetail.Order.Context != null && DigitalMarketingHelper.DIGITAL_MARKETING_CONTEXTID.Contains(orderDetail.Order.Context.Id) && orderDetail.OptionId.HasValue && orderDetail.OptionId.Value != 0)
                        order.Total += DigitalMarketingHelper.AD_WORDS_INCREMENT * orderDetail.OptionId.Value;
